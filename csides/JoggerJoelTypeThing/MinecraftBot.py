...
def vari():
    # global statements for enemies
    global creep, zombie

    # global statments for enemy health
    global creep_health, zombie_health

    # global staments for weapons
    global wood_sword, iron_sword, silver_sword, gold_sword, diamond_sword

    # global statements for enemies kill counters
    global creep_killcount, zombie_killcount

    # global statements for weapon amounts
    global wood_sword_amount, iron_sword_amount, silver_sword_amount, gold_sword_amount, diamond_sword_amount

    # global statements for items
    global work_bench

    # global statements for item amounts
    global work_bench_amount

    # global satements for blocks
    global wood_block, stone_block, coal_block, dirt_block, redstone_block, diamond_block, water_block, lava_block, iron_block
    global silver_block, lapis_lazuli_block, obsidian_block, gold_block

    # global statements for block amounts
    global wood_block_amount, stone_block_amount, dirt_block_amount, water_block_amount, obsidian_block_amount, lava_block_amount

    # global statements for ores
    global iron_ore, silver_ore, gold_ore, diamond_ore, redstone_ore, lapis_lazuli_ore

    # global statements for ore amounts
    global iron_ore_amount, silver_ore_amount, gold_ore_amount, diamond_ore_amount, redstone_ore_amount, lapis_lazuli_ore_amount
...
def worldinit():
    global resp  # makes it so the response variable can be used in the global scope of the program
    # This program takes user into a minecraft world called Gen
    # define functions within worldinit() function
    ...
    menu()  # Minecraft v 1.0.5 function
    resp = input()
    if resp == "c":
        ...
        # character creation
        print(
            "Notification: Before you can play your world you must make your username!"
        )
        user()
        ...
        print(
            ...
            + user
            + " start off by cutting some trees down for "
            + wood_block
        )
        print("(press enter)")
        input()
        ...
        input()
        print(
            "Notification: Great you collected enough wood to craft a work bench! with work benches you are able"
        )
        print(
            "to craft items with the resources you have to kill enemies! Lets craft a work bench."
        )
        print("(press enter)")
        input()
        ...
        input()
        print(
            "-You walk under a shallow tree and decide to place the work bench down and as you start to build and craft"
        )
        print(
            "the beautiful sun is gazing down at you..The moon is closing in and the cool midnight air is hitting your skin"
        )
        print(
            "the feeling of being enveloped by the world "
            + world_name
            + " is exhilerating yet calm.-"
        )
        time.sleep(5)
        print(
            "You finally finish the work bench and the wood is glossy and a soft tampered cloth quilted lay over it-"
        )
        print("(press enter)")
        input()
        print(
            "Notification: Great you have crafted the most common item in the game but did you know.."
        )
        time.sleep(2)
        print(
            "Notification: That with this work bench you can craft other items as well? lets give it a try"
        )
        print("(press enter)")
        input()
        print(
            "Notification: Enemy like the "
            + creep
            + " will try to halt you in your tracks"
        )
        print("maybe crafting a " + wood_sword + " would be choice!")
        time.sleep(1)
        print(
            "Notification: Your going to need some more wood you only have "
            + "("
            + str(wood_block_amount)
            + ") currently"
        )
        ...
        print(
            "Notification: Its night time and enemies are closing in craft a "
            + wood_sword
        )
        print("(press enter)")
        input()
        ...
        input()
        print(
            "-You pull out your work bench and place it on the soft pillow like block as you are in the snow biome"
        )
        print(
            "you hear the sounds of a sickening hissing sound in the forest below and the creepers are making their way"
        )
        ...
        print(
            "-You pull out your wooden sword and begin to slash and rip and tear at the creepers nearing up to you!-"
        )
        ...
        input("(press enter)")
        print(
            "-You walk towards the hills back up to your house and close the door and the screen starts to fade away you have done what no"
        )
        ...
        print(
            "-special thanks to everyone at twitch and shoutout to JoggerJoel for this to be made! you a legend-"
        )
        sys.exit()
